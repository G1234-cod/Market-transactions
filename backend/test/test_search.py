...
class TestSearch:
    ...
    def test_search_by_image(self, client, sample_image):
        """测试以图搜图"""
        files = {"image": ("test.jpg", sample_image, "image/jpeg")}
        data = {"top_k": 5}
        response = client.post("/api/v1/search/image", files=files, data=data)
    ...
    def test_search_by_text(self, client):
        ...
        response = client.post("/api/v1/search/text", data=data)
        assert response.status_code == 200
        result = response.json()
        assert result["success"] is True