<template>
  <div class="glass-card p-5 flex gap-5 hover:shadow-card-hover transition-all duration-300 group border-border/50">
    <div class="relative w-24 h-24 rounded-xl bg-space-lighter/30 flex-shrink-0 flex items-center justify-center overflow-hidden">
      <img
      ...
      <div class="absolute inset-0 bg-gradient-to-t from-space-card/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
    </div>
    <div class="flex-1 min-w-0">
      <div class="flex items-start justify-between gap-3">
        <h4 class="font-semibold text-text-primary truncate group-hover:text-primary-400 transition-colors">
      ...
      <div v-if="item.category || item.brand || item.model" class="flex items-center gap-2 mt-2 text-xs text-text-muted flex-wrap">
        <span v-if="item.category" class="px-2 py-0.5 rounded-md bg-primary-500/10 text-primary-400">{{ item.category }}</span>
        <span v-if="item.brand" class="px-2 py-0.5 rounded-md bg-accent-500/10 text-accent-400">{{ item.brand }}</span>
        <span v-if="item.model" class="px-2 py-0.5 rounded-md bg-space-lighter/50 text-text-secondary">{{ item.model }}</span>
        <span v-if="item.condition" class="text-text-light">· {{ item.condition }}</span>
      </div>

      <div class="flex items-center gap-4 mt-3 text-xs">
        <span v-if="item.suggested_price" class="text-amber-400 font-semibold font-variant-numeric">¥{{ formatPrice(item.suggested_price) }}</span>
        ...
        <div class="ml-auto flex gap-2">
          <button
            ...
            class="btn-secondary-sm text-xs flex items-center gap-1 ripple-container"
      ...
      <div v-if="reevaluateResult && reevaluateItemId === item.id"
        class="mt-3 bg-primary-500/10 border border-primary-500/30 rounded-xl p-3">
        <p class="text-xs text-primary-400 font-medium mb-1">✅ 重新评估完成</p>
        <p class="text-xs text-text-secondary">新标题：{{ reevaluateResult.title }}</p>
        <p v-if="reevaluateResult.suggested_price" class="text-xs text-amber-400 font-semibold mt-1">
      ...
      <div v-if="reevaluateError && reevaluateItemId === item.id"
        class="mt-3 bg-danger-500/10 border border-danger-500/30 rounded-xl p-3">
        <p class="text-xs text-danger-400">❌ 重新评估失败：{{ reevaluateError }}</p>
      </div>
    </div>
  </div>
</template>
...
function statusClass(status) {
  return status === 'published' ? 'bg-accent-500/20 text-accent-400 border border-accent-500/30'
    : status === 'delisted' ? 'bg-text-muted/20 text-text-secondary border border-transparent'
    : status === 'sold' ? 'bg-success-500/20 text-success-400 border border-success-500/30'
